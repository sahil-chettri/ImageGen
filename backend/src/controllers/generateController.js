...
import { generateImage, transformImage, inpaintImage, enhanceImage } from '../services/aiService.js';
import { embedImage } from '../services/ragService.js';
...
/* ── Helpers ─────────────────────────────────────────────────────────────── */
async function deductCredits(userId, cost = CREDIT_COST) {
...
/** POST /api/v1/generate/enhance (multipart: image) */
export async function enhanceToHD(req, res, next) {
  try {
    if (!req.file)
      return res.status(400).json({ success: false, message: 'Image file is required' });

    const scaleFactor = parseInt(req.body.scaleFactor, 10) || 2;
    if (![2, 4, 8].includes(scaleFactor))
      return res.status(400).json({ success: false, message: 'scaleFactor must be 2, 4, or 8' });

    // 8× runs two AI passes so costs 2 credits; 2× and 4× cost 1
    const creditCost       = scaleFactor >= 8 ? 2 : 1;
    const creditsRemaining = await deductCredits(req.user.id, creditCost);

    const sourceImageUrl = `/uploads/${req.file.filename}`;

    const result = await enhanceImage({ imageFile: req.file, scaleFactor });

    const generation = await saveGeneration({
      userId:         req.user.id,
      type:           'enhancement',
      prompt:         `AI Enhancement ${scaleFactor}x upscale`,
      negativePrompt: '',
      ratio:          '1:1',
      style:          'Photorealistic',
      sourceImageUrl,
