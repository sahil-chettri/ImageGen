    ...
    <div style={{ height: "100vh", width: "100vw", display: "flex", flexDirection: "column", overflow: "hidden", background: T.cream, fontFamily: "'DM Sans', sans-serif", color: T.ink, position: "relative" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ratio btn */
        .i2i-ratio { padding:6px 14px; border-radius:8px; border:1px solid ${T.border}; background:rgba(245,240,232,0.5); color:${T.inkMuted}; font-size:11px; font-weight:500; cursor:pointer; display:flex; flex-direction:row; align-items:center; gap:6px; transition:all 0.15s; font-family:'DM Sans',sans-serif; flex-shrink:0; white-space:nowrap; }
      ...
      <div style={{ flex:1, display:"flex", gap:18, padding:"18px 40px 18px", overflow:"hidden", position:"relative", zIndex:1, alignItems:"stretch" }}>
              ...
              <div style={{ display:"flex", gap:7, justifyContent:"flex-start" }}>
                {RATIOS.map(r => (
                  <button key={r.label} className={`i2i-ratio ${ratio===r.label?"i2i-ratio-on":""}`} onClick={() => setRatio(r.label)}>
                    <div style={{ width: Math.round(12*Math.min(1, r.w/r.h)), height: Math.round(12*Math.min(1, r.h/r.w)), border:`1.5px solid ${ratio===r.label?T.accent:T.inkMuted}`, borderRadius:1, background:ratio===r.label?T.accentDim:"transparent", transition:"all .15s", flexShrink:0 }} />
                    {r.label}
                  </button>
                ))}
              </div>
        ...
        <div className="i2i-panel" style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minHeight:0 }}>
          ...
          <div style={{ flex:1, display:"flex", minHeight:0, overflow:"hidden", background:"rgba(245,240,232,0.35)", padding:"12px 16px", gap:12 }}>
            ...
            <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, minHeight:0 }}>
              ...
              {/* Image box */}
              <div style={{
                flex:1, minHeight:"300px", borderRadius:14, overflow:"hidden",
                ...
                {!loading && image && (
                  <img src={image} alt="Transformed" onLoad={() => setImgLoaded(true)}
                    style={{ width:"100%", height:"100%", objectFit:"contain", display:"block", opacity:imgLoaded?1:0, transition:"opacity 0.45s ease", animation:"i2i-reveal 0.5s ease", minHeight:"300px" }} />
                )}
              </div>
            </div>
          </div>
